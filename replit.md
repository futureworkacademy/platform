...
Essay prompts use structured 3-part sub-questions (Evidence-Based Rationale, Stakeholder Trade-offs, Risk Mitigation) with explicit source code references (AIM, APX, WFT, etc.) and recommended reading panels. Word counts: 250 min for W1/W2, 300 min for W3, 150 min for rationale. Four rubric criteria (Evidence Quality, Reasoning Coherence, Trade-off Analysis, Stakeholder Consideration, 25 pts each) are consistently defined across `shared/schema.ts` (defaultRubricCriteria), `server/grading-page-renderer.ts` (GRADING_RUBRIC), `client/src/pages/for-educators.tsx` (RUBRIC_CRITERIA), and `server/services/llm-evaluation.ts` (dynamic via callers). The rubric and recommended sources are always visible on the decision page alongside the essay input. The LLM grading prompt in `server/services/llm-evaluation.ts` includes explicit scoring calibration guidance so that excellent responses (specific data citations, multi-stakeholder analysis, risk mitigation) score 93-96%, good responses score 72-88%, adequate responses score 52-68%, and poor responses score below 52%. The `GRADING_RUBRIC` evaluation guidelines in `server/grading-page-renderer.ts` use granular scoring bands (24-25, 21-23, 15-20, 10-14, <10) to reduce variance and reward strong work appropriately. An exemplar response (Week 1, Option C) is stored in `grading_reports` for instructor calibration reference.
