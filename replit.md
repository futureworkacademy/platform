...
**External Grading Module (Mar 2026):** Public server-rendered page at `/grade` (`server/grading-page-renderer.ts`) enables instructors to grade student responses submitted through Blackboard or other LMS platforms without requiring students to be enrolled in the system. Three tabs: (1) Single Response — paste one student's essay, select week and option chosen, get AI-scored rubric feedback instantly with instructor comments and PDF export; (2) Bulk Upload — upload a CSV (columns: StudentName, OptionChosen, EssayText) to grade multiple responses sequentially with progress tracking and downloadable results CSV; (3) Grading History — browse, review, and re-download saved grading reports with inline comment editing. Uses the same `evaluateTextResponse` LLM engine from `server/services/llm-evaluation.ts`. Scoring rubric: Strategic Thinking (25pts), Stakeholder Awareness (25pts), Risk Assessment (25pts), Research Application (25pts) = 100pts total. Rate-limited to 10 requests/minute per IP. CSV parser handles multiline quoted fields for essay text. Linked from Week 0 Orientation page under "Instructor Tools" section.

**Curved Scoring System (Mar 2026):** Dynamic Z-score normalization applied to saved grading reports. Formula: `curvedScore = 75 + (rawPct - weekMean) × (10 / weekStdDev)`, clamped to [40, 100]. Requires at least 3 saved reports per week to activate. Curved scores are computed dynamically at display time (not stored in DB) so they update automatically as more submissions are graded. Displayed in single result card, history table, history detail expansion, and PDF reports alongside raw scores. Week statistics (mean, std dev, submission count) shown for context. API endpoints return `curvedScore` and `weekStats` fields.
